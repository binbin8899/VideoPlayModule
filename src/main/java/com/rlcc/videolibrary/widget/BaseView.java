...
import com.rlcc.videolibrary.listener.ExoPlayerListener;
import com.rlcc.videolibrary.listener.OnEndGestureListener;
import com.rlcc.videolibrary.listener.PLayEpisodeListener;
...
abstract class BaseView extends FrameLayout {
    ...
    /***错误页,进度控件,锁屏按布局,自定义预览布局,提示布局,播放按钮,弹幕,右侧控件（截图，弹幕）*/
    protected View exoLoadingLayout, exoPlayPreviewLayout, exoPreviewPlayBtn, exoBarrageLayout,exoControllerRight;
    /***水印,封面图占位,显示音频和亮度布图,返回按钮,截图*/
    protected ImageView exoPlayWatermark, exoPreviewImage, exoBottomPreviewImage, exoControlsBack,exoFullscreenView;
    /***手势管理布局view***/
    ...
    /***广告布局***/
    protected final AdControlView mAdControlView;
    /***切换***/
     ...
     * 选集
     */
    protected SelectionView mSelectionView;
    protected int speedIndex = 1;//倍速
    protected int selectionIndex;//选集第几集
    /***流量提示框***/
    ...
    private int ic_fullscreenImage = R.drawable.ic_fullscreen_white;
    ...
    //电视剧选择时回调
    protected PLayEpisodeListener mPLayEpisodeListener;
    //是否广告
    ...
    public BaseView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        exoControllerRight = playerView.findViewById(R.id.exo_controller_right);
        ...
        mAdControlView = new AdControlView(getContext(), attrs, defStyleAttr,this);
    ...
    private void intiView() {
        ...
        //全屏按钮
        exoFullscreenView = new ImageView(getContext());
        exoFullscreenView.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
        exoFullscreenView.setId(R.id.exo_video_fullscreen);
        exoFullscreenView.setImageDrawable(ContextCompat.getDrawable(getContext(), ic_fullscreenImage));
        exoFullscreenView.setPadding(ss, ss, ss, ss);
        ...
        frameLayout.addView(mAdControlView,frameLayout.getChildCount());
        ...
        layoutParams.topMargin = VideoPlayUtils.dip2px(getContext(), 26f);
        frameLayout.addView(exoControlsBack, frameLayout.getChildCount(), layoutParams);

        LayoutParams layoutParamsfs = new LayoutParams(VideoPlayUtils.dip2px(getContext(), 35f), VideoPlayUtils.dip2px(getContext(), 35f));
        layoutParams.topMargin = VideoPlayUtils.dip2px(getContext(), 26f);
        layoutParamsfs.gravity = Gravity.RIGHT|Gravity.BOTTOM;
        frameLayout.addView(exoFullscreenView, frameLayout.getChildCount(), layoutParamsfs);
...
//        }
    ...
    public void showFullscreenView(int visibility) {
        if (getExoFullscreen() != null) {
            if (isLand) {
                getExoFullscreen().setVisibility(GONE);
            } else {
                if (visibility == VISIBLE) {
                    getExoFullscreen().setTranslationY(0);
                    getExoFullscreen().setAlpha(1f);
                }else{
                }
                getExoFullscreen().setVisibility(visibility);
     ...
     */
    public void setOnPlayClickListener(@Nullable OnClickListener onClickListener) {
...
//        return controllerView.getExoFullscreen();
     ...
     */
    @NonNull
    public TextView getSelectionText() {
        return controllerView.getSelectionText();
    }
    ...
    public TextView getSpeedText() {
        return controllerView.getSpeedText();
    }
     ...
     */
    @NonNull
    public TextView getSkipTitleText() {
        return controllerView.getSkipTitleText();
    }
     ...
     */
    @NonNull
    public ImageButton getNextEpisode() {
        return controllerView.getNextEpisode();
    }
//    /**
//     * 更多
//     *
...
//        return controllerView.getMoreImage();
//    }
//
//    /**
//     * 分享H5视频地址
...
//        return controllerView.getShare();
//    }
    /***
     * 获取预览图
     *
     ...
     */
    @NonNull
    public ExoDefaultTimeBar getLandTimeBar() {
        return (ExoDefaultTimeBar) controllerView.getLandTimeBar();
    }
    /**
     * 横屏显示占位空间
     *
     */
    @NonNull
    public View getLandWeight() {
        return (View) controllerView.getLandWeight();
    }


    /**
     * @deprecated 返回水印
     * @return exoPlayWatermark
     */
    public ImageView getPlayWatermark(){
        return exoPlayWatermark;
    }
    /**
     * 时间分隔符
     ...
     */
    @NonNull
    public TextView getMediaTimeSpan() {
        return controllerView.getMediaTimeSpan();
    }

    protected final OnTouchListener mOnTouchListener = new OnTouchListener() {
        @Override
        public boolean onTouch(View v, MotionEvent event) {
            android.util.Log.e("onTouch>>>-->>BaseView", event.getAction() + "");
            if (!controllerHideOnTouch) {
                //手势禁用时（广告时间）
                showBackView(View.VISIBLE,true);
//                getPlaybackControlView().show();
//                getPlaybackControlView().setInAnim();
//                getExoFullscreen().setVisibility(View.VISIBLE);

                return false;
            } else if (mLockControlView.isLock()) {
                getPlaybackControlView().setInAnim();
                getPlayerView().maybeShowController(true);
                return false;
            }
//            else if (!isLand) {
//                //竖屏不执行手势
...
//                return true;
    ...
    public void setEpisodes(PLayEpisodeListener episodeListener,List<Episode> list) {
        this.episodes = list;
        ...
        this.mPLayEpisodeListener = episodeListener;
    }
    protected int getSpeedIndex() {
        return speedIndex;
    }

    protected void setSpeedIndex(int speed) {
