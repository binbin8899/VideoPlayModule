...
import com.rlcc.videolibrary.widget.BaseView;
import com.rlcc.videolibrary.widget.ExoDefaultTimeBar;
...
public class LockControlView extends FrameLayout implements View.OnClickListener, AnimUtils.AnimatorListener, AnimUtils.UpdateProgressListener {
    ...
    public void showLockState(int visibility) {
        if (exoPlayLockLayout != null) {
            if (mBaseView.isLand()) {
                ...
                exoControllerRight.setVisibility(visibility);
            } else {
                lockCheckBox.setVisibility(GONE);
            }
            if (isProgress) {
    ...
    public void show(boolean isIn) {
        ...
        if (isIn) {
            ...
            if (exoControllerRight != null && mBaseView.isLand()) {
