...
public class ToastUtils {
    ...
    public static void showLongToastSafe(final CharSequence text) {
        sHandler.post(new Runnable() {
            @Override
            public void run() {
                showToast(0,text, Toast.LENGTH_LONG);
            }
        });
    }
     ...
     * @param duration 显示时长
     */
    private static void showToast(int res,CharSequence text, int duration) {
        ...
        try {
            ...
            Context cxt = application.getBaseContext();
            if(res!=0&&TextUtils.isEmpty(text)){
                text = application.getApplicationContext().getResources().getText(res);
            }
            sToast = Toast.makeText(application.getApplicationContext(), text, duration);
            sToast.show();
